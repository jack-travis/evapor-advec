...
class core:
    ...
    def step(self):
        ...
        for i in range(self.gs):
            E_n = self.E[i]
            #if not enought vapour/liquid is present, reduce E proportionally
            if E_n > 0.0:
            ...
            if E_n < 0.0:
                if v_n[i] < -E_n*self.Dt:
                    E_n = v_n[i] / self.Dt
##            #in case of erroneous large negative values, cap
##            if E_n < -self.E[i]*self.Dt:
##                E_n = -self.E[i]*self.Dt
##            if E_n > self.E[i]*self.Dt:
##                E_n = self.E[i]*self.Dt
